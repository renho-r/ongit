...
public class TestHttpClient {
	...
	public static void main(String[] args) {
		...
		// 创建get方法的实�?
		GetMethod getMethod = new GetMethod("http://192.168.0.104/interf/servlet/RecommendAction");
		// 使用系统提供的默认恢复策�?
		// getMethod.getParams().setParameter(HttpMethodParams.RETRY_HANDLER,
		// new DefaultHttpMethodRetryHandler());

		// 创建post方法实例
		PostMethod postMethod = new UTF8PostMethod("http://192.168.0.104/interf/servlet/RecommendAction");
		//  
		// //填入各个表单域的�?
		...
		try {
			...
			// 执行post方法
			NameValuePair[] data1 = {
					new NameValuePair("type", "cartoon"),
			...
			if (statusCode == HttpStatus.SC_MOVED_TEMPORARILY) {
				...
				postMethod.setRequestHeader("Referer", "http://localhost:80/itv2.0.login");
//				NameValuePair[] data1 = {
//						new NameValuePair("type", "cartoon"),
//						new NameValuePair("password", "password") };
				postMethod.setRequestBody(data1);
