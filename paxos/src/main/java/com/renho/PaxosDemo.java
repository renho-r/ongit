...
public final class PaxosDemo {
     ...
     * @return
     */
    private static Proposal nextProposal(long currentVoteNumber, List proposals) {
        long voteNumber = currentVoteNumber + 1;
        if (proposals.isEmpty()) {
            return new Proposal(voteNumber, PROPOSALS[RANDOM.nextInt(PROPOSALS.length)]);
        }
        Collections.sort(proposals);
        Proposal maxVote = (Proposal) proposals.get(proposals.size() - 1);
    ...
    private static class Proposer {
         ...
         * @param acceptors
         */
        public static void vote(Proposal proposal, Collection acceptors) {
            int quorum = Math.floorDiv(acceptors.size(), 2) + 1;
            int count = 0;
            while (true) {
                printInfo("VOTE_ROUND", "START", ++count + "");
                List proposals = new ArrayList();
                for (Object acceptorObj : acceptors) {
                    Acceptor acceptor = (Acceptor) acceptorObj;
...
//假设这个过程有50%的几率失败
    ...
    private static class Proposal implements Comparable {
        ...
        public int compareTo(Object o) {
            ...
            return Long.compare(voteNumber, proposal.voteNumber);
        }
//        @Override
