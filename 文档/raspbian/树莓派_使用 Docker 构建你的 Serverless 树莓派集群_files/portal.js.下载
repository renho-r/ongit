...
function portal_comment_reppost(cid, aid) {
	var x = new Ajax();
	x.get('portal.php?mod=portalcp&ac=comment&op=reppost&cid='+cid+'&aid='+aid+'&inajax=1', function(s){
		var data = s.split('||reppost||',2);
		ajaxinnerhtml($('rep_username'), data[0]);
		ajaxinnerhtml($('rep_content'), '<blockquote>'+data[1]+'</blockquote>');
		$('reppid').value = cid;
...
function block_load_html(id, count) {
	$(id).innerHTML += '<div id="'+id+'_tmp"><center>正在加载内容...</center></div>';
	var x = new Ajax();
	x.get('portal.php?mod=portalcp&ac=index&op=getblock&inajax=1&count='+count, function(content) {
		$(id).removeChild($(id+'_tmp'));
		if (content.length > 0) {
			content = '<div class="xfs xfs_1 frame move-span frame-1" style="border: #CCC 1px solid !important;margin: 0 0 5px 0 !important;">'
				+ '<div class="column frame-1-c" ><div class="move-span temp"></div>' 
				+ content
				+ '</div></div>';
...
function menuAnchor(menu){
	for( k in menu ){
		//jQuery("h"+menu[k]['level']+":contains('"+menu[k]['title']+"')").attr("id",menu[k]['level']+'_'+menu[k]['position']);
		jQuery("h"+menu[k]['level']+":contains('"+menu[k]['title']+"')").before('<a id="'+menu[k]['level']+'_'+menu[k]['position']+'" class="target-fix"></a>');
		if( menu[k]['child'].length>0 ){
			menuAnchor(menu[k]['child']);
		}
	}
}

function menuFolder(obj){
	if( obj.innerHTML=='-' ){
		obj.innerHTML = '+';
		jQuery(obj).parent().parent().children("ul").slideUp();
	}else{
		obj.innerHTML = '-';
		jQuery(obj).parent().parent().children("ul").slideDown();
	}
}