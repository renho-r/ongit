...
feedListGet = function(ele){
      ...
      var feedUrlSource = thisFeedLi.find('a').attr('strategy')||"";
...
feedlistScroll= function(){
      ...
      viewedHeight = top; 
      feedlist =$(document).find('.tracking-ad[data-poputype$="feed"]'); 
      for(i = 0; i<feedlist.length; i++){
        feedchild = feedlist[i];
        var childListTop = $(feedlist[i]).offset().top;
        feedListGet();
        if (($(feedlist[i]).attr('data-feed-show')==="false")&&(currentBottom>=childListTop)) { 
  ...
  (function(definition, undef) {
    ...
    scrollHandle = function(){
      ...
      for (_i = 0, _len = ads.length; _i < _len; _i++) {
        ad = ads[_i];
        if ((!ad.viewed)&&(currentBottom>ad.top)&&(ad.top>scrollH)) {     
    ...
    feedlistClick = function(){
      var feedlist = $(this).parents(".tracking-ad");
