    ...
    var oBlog = {
        ...
        leftMenu:function(){
            ...
            $('#comment_list').on('click','.rec_number',function(){
                ...
                if(com_child_listshow==='false'){
                    arrow.addClass("icon-shangjiantou").removeClass("icon-xiajiantou");
                    ...
                    arrow.addClass("icon-xiajiantou").removeClass("icon-shangjiantou");
        ...
        fixedWrap:function(){
            ...
            $(window).scroll(function(){
                ...
                if(winH >= wrapH){
                    if(scrollTop-fixTop >=0){
                        $(".fixRight").css({'position':'fixed','top':'0','left':baiduBoxLeft});
                    }else{
                        $(".fixRight").css({'position':'relative','top':'auto','left':'auto'});
                    }
                }else{
                    if(scrollTop-fixTop >=0){
                        subFirst.hide();
...
function noComments() {
    $(listOutBoxClass).html('<div style="display:none"><br />&nbsp;&nbsp;暂无评论<br /><br /><div class="clear"></div></div>');
}
function loadList(pageIndex, isSub) {
    ...
    }else
    {
        ...
        var cmtUrl = "http://"+url[2]+"/"+url[3]+"/comment/list/" + fileName + "?page=" + pageIndex +"&size=6";
        ...
        $.get(cmtUrl, function (json) {
            ...
            $("#comment_bar").html('<a id="load_comments" class="btn btn-large more_comment_btn" page="' + (pageIndex + 1) + '">查看 <span>'+RecordCount +'</span> 条热评<i class="icon iconfont icon-xiajiantou"></i></a>');
            //添加按钮事件
            setBtnEvent();
            outoBlog.MoreComment();
            //scrollInit();
...
//    html += '                     <a href="#report" class="com_reply" title="举报" commentid="' + comment.CommentId + '" floor="' + index + '">举报</a>';
    ...
    html += '                     <button class="btn btn-noborder rec_number">' + comment.Replies.length + '条回复<i class="icon iconfont icon-xiajiantou"></i></button>';
    }
...
$(function(){
  ...
  function newColumnTabSwitch(e){
    ...
    if ($($obj).is(".recommend_btn,.recommend_btn *")){
      ...
      $new_btn.css('border-bottom', '2px solid #d9dfe6');
    }else if($($obj).is(".new_btn,.new_btn *")){
      $recommendbox.fadeOut(300);
      $newbox.fadeIn(300);
      $new_btn.css('border-bottom', '2px solid #f00');
      $recommend_btn.css('border-bottom', '2px solid #d9dfe6');
    }
  }
