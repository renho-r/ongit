...
public class P212 {
                ...
                if (!result.containsKey(words[i])) {
                    for (int n = 0; n < colCount; n++) {
                        if (!result.containsKey(words[i])) {
                            ...
                            allReturn = false;
                            dfs(board, words[i], m, n, result, visited, "");
                        } else {
                            break;
                        }
                    }
    ...
    private void dfs(char[][] board, String curStr, int row, int col, Map<String, String> result, boolean[][] visited, String tmpStr) {
        if (allReturn) {
            return;
        }
        ...
        if (!curStr.startsWith(tmpStr)) {
            return;
        }
        visited[row][col] = true;
        dfs(board, curStr, row - 1, col, result, visited, tmpStr);
        if (allReturn) {
            return;
        }
        dfs(board, curStr, row + 1, col, result, visited, tmpStr);
        ...
        dfs(board, curStr, row, col - 1, result, visited, tmpStr);
        if (allReturn) {
            return;
        }
        dfs(board, curStr, row, col + 1, result, visited, tmpStr);
